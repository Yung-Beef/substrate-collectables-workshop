...
#[test]
fn do_abandon_removes_from_storage() {
	new_test_ext().execute_with(|| {
		let kitty_id = [1u8; 32];
		let _ = PalletKitties::mint(ALICE, kitty_id);
		assert_eq!(Kitties::<TestRuntime>::contains_key(kitty_id), true);
		let _ = PalletKitties::abandon(RuntimeOrigin::signed(ALICE), kitty_id);
		assert_eq!(Kitties::<TestRuntime>::contains_key(kitty_id), false);
	})
}
